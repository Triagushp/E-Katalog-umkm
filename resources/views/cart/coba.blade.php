                        ...
                        <div class="store-group" data-umkm-id="{{ $umkmId }}">
                            ...
                            <div class="cart-item" data-item-id="{{ $item->id }}" data-umkm-id="{{ $umkmId }}">
                                <div class="item-selector">
                                    <label class="checkbox">
                                        <input type="checkbox" class="item-checkbox" data-price="{{ $item->product->price }}" data-quantity="{{ $item->quantity }}" checked>
                                        <span class="checkmark"></span>
                                    </label>
                                </div>
                            ...
                            <div class="store-summary">
                                ...
                                <form action="" method="POST" class="store-checkout-form">
                                    ...
                                    <button type="submit" class="store-checkout-btn">Checkout {{ $umkmName }}</button>
                                </form>
                            </div>
                        </div>
                    @endforeach
...
<script>
    // Required JavaScript for the shopping cart functionality
...
$(document).ready(function() {
    ...
    $('.quantity-btn').on('click', function() {
        ...
        const itemPrice = parseFloat($(this).closest('.cart-item').find('.item-checkbox').data('price'));
        const itemTotal = itemPrice * quantity;
        $('#total-' + itemId).text('Rp' + formatNumber(itemTotal));
        
        // Update item checkbox data attribute
        $(this).closest('.cart-item').find('.item-checkbox').data('quantity', quantity);
        ...
        $.ajax({
            ...
            error: function(error) {
                ...
                quantityElement.text(quantity - (isIncrease ? 1 : -1));
                updateCartTotals();
            }
        });
    });
    ...
    $('.item-checkbox').on('change', function() {
        const storeCheckbox = $(this).closest('.store-group').find('.store-checkbox');
        const allItemCheckboxes = $(this).closest('.store-group').find('.item-checkbox');
        const checkedItemCheckboxes = $(this).closest('.store-group').find('.item-checkbox:checked');
    ...
    $('.store-checkbox').on('change', function() {
        ...
        storeGroup.find('.item-checkbox').prop('checked', isChecked);
    ...
    $('.shipping-select').on('change', function() {
        ...
        $('.store-shipping[data-umkm-id="' + umkmId + '"]').text('Rp' + formatNumber(shippingCost));
        
        updateStoreTotals(umkmId);
        updateCartTotals();
    });
    
    // Initialize checkout forms with selected items
    updateCheckoutForms();
    
    // Handle voucher button clicks
    $('.voucher-btn').on('click', function() {
        const umkmId = $(this).data('umkm-id');
        
        // Show voucher modal (implementation would depend on your modal system)
        $('#voucher-modal').modal('show');
        $('#selected-umkm').val(umkmId);
    });
    ...
    function updateCartTotals() {
        ...
        $('.item-checkbox:checked').each(function() {
            ...
            const umkmId = $(this).closest('.store-group').data('umkm-id');
            
            totalItems += quantity;
            subtotalPrice += price * quantity;
            selectedStores.add(umkmId);
        });
        
        // Calculate shipping based on selected stores
        selectedStores.forEach(function(umkmId) {
            const shippingCost = parseInt($('.shipping-select[data-umkm-id="' + umkmId + '"]').val());
        ...
        $('#total-stores').text(selectedStores.size + ' Toko');
        $('#shipping-cost').text('Rp' + formatNumber(totalShipping));
        
        // Update per-store totals
    ...
    // Function to update per-store totals
    function updateStoreTotals(umkmId) {
        ...
        const shippingCost = parseInt($('.shipping-select[data-umkm-id="' + umkmId + '"]').val() || 0);
        const discount = parseInt($('.store-discount[data-umkm-id="' + umkmId + '"]').text().replace(/[^\d]/g, '') || 0);
        ...
        $('.store-total-price[data-umkm-id="' + umkmId + '"]').text('Rp' + formatNumber(storeSubtotal + shippingCost - discount));
    }
    
    // Function to update checkout form data
    function updateCheckoutForms() {
        $('.store-group').each(function() {
            const umkmId = $(this).data('umkm-id');
            const selectedItems = [];
            
            // Get all checked items for this store
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize variables to track cart state
    let cartState = {
        items: {},
        shippingCosts: {},
        vouchers: {},
    ...
    // Store checkbox event listeners
    document.querySelectorAll('.store-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            const umkmId = this.dataset.umkmId;
            const isChecked = this.checked;
            
            // Update all item checkboxes within this store
            document.querySelectorAll(`.cart-item[data-umkm-id="${umkmId}"] .item-checkbox`).forEach(itemCheckbox => {
    ...
    document.querySelectorAll('.shipping-select').forEach(select => {
        select.addEventListener('change', function() {
            const umkmId = this.dataset.umkmId;
            cartState.shippingCosts[umkmId] = parseInt(this.value);
            updateCartSummary();
        });
    });
    
    // Apply voucher button event
    document.querySelector('.apply-voucher').addEventListener('click', function() {
        const voucherCode = document.querySelector('.voucher-input').value.trim();
        if (voucherCode) {
            applyGlobalVoucher(voucherCode);
        }
    });
    
    // Store voucher buttons event
    document.querySelectorAll('.voucher-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            const umkmId = this.dataset.umkmId;
            showStoreVoucherModal(umkmId);
        });
    });
    ...
    document.querySelector('form[action*="checkout.process"]').addEventListener('submit', function(e) {
        ...
        // Set the hidden input with cart data
        document.getElementById('cart-items-input').value = JSON.stringify({
            items: selectedItems,
            shipping: cartState.shippingCosts,
            vouchers: cartState.vouchers,
            globalVoucher: cartState.globalVoucher,
            globalVoucherDiscount: cartState.globalVoucherDiscount
        });
    ...
    function initializeCart() {
        document.querySelectorAll('.cart-item').forEach(item => {
            ...
            const umkmId = item.dataset.umkmId;
        ...
        document.querySelectorAll('.shipping-select').forEach(select => {
            const umkmId = select.dataset.umkmId;
            cartState.shippingCosts[umkmId] = parseInt(select.value);
        });
    ...
    function updateItemQuantity(itemId, isIncrease) {
        ...
        cartState.items[itemId].quantity = quantity;
        
        // Update item checkbox data attribute for calculations
        ...
        const totalPrice = quantity * cartState.items[itemId].price;
        itemTotal.textContent = `Rp${formatNumber(totalPrice)}`;
    ...
    function updateCartSummary() {
        ...
        Object.entries(cartState.items).forEach(([itemId, item]) => {
            const checkbox = document.querySelector(`.cart-item[data-item-id="${itemId}"] .item-checkbox`);
            if (checkbox && checkbox.checked) {
                subtotal += item.price * item.quantity;
                totalItems += item.quantity;
                cartState.items[itemId].selected = true;
            } else if (checkbox) {
                cartState.items[itemId].selected = false;
            }
        });
        
        // Calculate shipping total for selected stores
        let shippingTotal = 0;
        const selectedStores = new Set();
        
        Object.entries(cartState.items).forEach(([itemId, item]) => {
            if (item.selected) {
                selectedStores.add(item.umkmId);
            }
        });
        
        selectedStores.forEach(umkmId => {
            shippingTotal += cartState.shippingCosts[umkmId] || 0;
        });
        
        // Calculate voucher discounts
        let voucherDiscount = 0;
        
        // Apply store-specific vouchers
        Object.entries(cartState.vouchers).forEach(([umkmId, voucher]) => {
            if (selectedStores.has(umkmId)) {
                // Calculate store subtotal
                let storeSubtotal = 0;
                Object.entries(cartState.items).forEach(([itemId, item]) => {
                    if (item.selected && item.umkmId === umkmId) {
                        storeSubtotal += item.price * item.quantity;
                    }
                });
                
                // Apply voucher discount formula
                if (voucher.type === 'percentage') {
                    voucherDiscount += storeSubtotal * (voucher.value / 100);
        ...
        // Apply global voucher if available
        if (cartState.globalVoucher && cartState.globalVoucherDiscount > 0) {
            voucherDiscount += cartState.globalVoucherDiscount;
        }
        
        // Calculate grand total
        const grandTotal = subtotal + shippingTotal - voucherDiscount;
        
        // Update UI elements
        document.getElementById('total-items').textContent = totalItems;
        document.getElementById('subtotal-price').textContent = `Rp${formatNumber(subtotal)}`;
        document.getElementById('shipping-cost').textContent = `Rp${formatNumber(shippingTotal)}`;
        document.getElementById('discount-amount').textContent = `Rp${formatNumber(voucherDiscount)}`;
        document.getElementById('grand-total').textContent = `Rp${formatNumber(grandTotal)}`;
    }
    
    // Update store checkboxes based on item selections
    function updateStoreCheckboxes() {
        document.querySelectorAll('.store-checkbox').forEach(storeCheckbox => {
            const umkmId = storeCheckbox.dataset.umkmId;
            const itemCheckboxes = document.querySelectorAll(`.cart-item[data-umkm-id="${umkmId}"] .item-checkbox`);
            ...
            storeCheckbox.indeterminate = !allChecked && !allUnchecked;
        });
    }
    
    // Apply global voucher
    function applyGlobalVoucher(code) {
        // Simulate voucher validation
        // In a real app, you'd make an AJAX request to validate the voucher code
        ...
        const sampleVouchers = {
            ...
            'HEMAT50K': { type: 'fixed', value: 50000, message: 'Diskon Rp50.000 berhasil diterapkan' },
            'GRATIS': { type: 'shipping', value: 100, message: 'Gratis ongkir berhasil diterapkan' }
        };
        
        if (sampleVouchers[code]) {
            voucherValid = true;
            const voucher = sampleVouchers[code];
            ...
            if (voucher.type === 'percentage') {
                ...
                // Make shipping free
                voucherDiscount = 0; // We'll handle this differently in shipping calculation
                document.getElementById('shipping-cost').textContent = 'Rp0';
            }
            
            voucherMessage = voucher.message;
            cartState.globalVoucher = code;
            cartState.globalVoucherDiscount = voucherDiscount;
        } else {
    ...
    // Show store voucher modal
    function showStoreVoucherModal(umkmId) {
        // In a real app, you'd fetch available vouchers for this store via AJAX
        // For demo purposes, we'll show a simple prompt
        const sampleVouchers = [
            { code: 'TOKO10', type: 'percentage', value: 10, description: 'Diskon 10% untuk semua produk' },
            { code: 'TOKO25K', type: 'fixed', value: 25000, description: 'Potongan Rp25.000' }
        ];
        
        const voucherOptions = sampleVouchers.map((v, i) => `${i+1}. ${v.code} - ${v.description}`).join('\n');
        const selectedOption = prompt(`Pilih voucher untuk toko ini:\n${voucherOptions}\n\nMasukkan nomor voucher:`);
        
        if (selectedOption && !isNaN(selectedOption)) {
            const voucherIndex = parseInt(selectedOption) - 1;
            if (voucherIndex >= 0 && voucherIndex < sampleVouchers.length) {
                const selectedVoucher = sampleVouchers[voucherIndex];
                cartState.vouchers[umkmId] = selectedVoucher;
                
                // Update button text
                const voucherBtn = document.querySelector(`.voucher-btn[data-umkm-id="${umkmId}"]`);
                voucherBtn.textContent = `${selectedVoucher.code} Diterapkan`;
                voucherBtn.style.backgroundColor = 'var(--bg-blue)';
                
                alert(`Voucher ${selectedVoucher.code} berhasil diterapkan`);
                updateCartSummary();
            }
        }
    }
